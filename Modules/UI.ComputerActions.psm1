                ...
                $DoRefresh = {
                    ...
                    try {
                        if ($idx -eq 0) {
                            ...
                            $resProcs = @(if ($rawProcs) { foreach ($r in $rawProcs) { [PSCustomObject]@{ Name = $r.Name; Id = $r.Id; CPU = $r.CPU; MemMB = $r.MemMB; Description = $r.Description } } })
                            ...
                            $resSvcs = @(if ($rawSvcs) { foreach ($s in $rawSvcs) { [PSCustomObject]@{ Name = $s.Name; DisplayName = $s.DisplayName; State = $s.State; StartMode = $s.StartMode } } })
                            ...
                            $resProfs = @(if ($rawProfs) { foreach ($p in $rawProfs) { [PSCustomObject]@{ LocalPath = $p.LocalPath; LastUseTime = $p.LastUseTime; Loaded = $p.Loaded; SID = $p.SID } } })
                            ...
                            $resDevs = @(if ($rawDevs) { foreach ($d in $rawDevs) { [PSCustomObject]@{ FriendlyName = $d.FriendlyName; Class = $d.Class; Status = $d.Status; Manufacturer = $d.Manufacturer; InstanceId = $d.InstanceId } } })
                            ...
                            if (-not [string]::IsNullOrWhiteSpace($targetPath)) {
                                ...
                                $resFiles = @(
                                    if ($rawFiles) {
                                        foreach ($f in $rawFiles) { [PSCustomObject]@{ Name = $f.Name; ItemType = $f.ItemType; Size = $f.Size; LastWriteTime = $f.LastWriteTime; FullName = $f.FullName } }
                                    }
                                )
                                if ($lvFiles) {
                        ...
                        elseif ($idx -eq 7) {
                            ...
                            $resEvts = @(if ($rawEvts) { foreach ($e in $rawEvts) { [PSCustomObject]@{ TimeCreated = $e.TimeCreated; Level = $e.LevelDisplayName; Id = $e.Id; Source = $e.ProviderName; Message = if ($e.Message) { $e.Message -replace "`r", "" -replace "`n", "  " } else { "" } } } })
                ...
                if ($btnRefresh -and $dgSoftware) {
                    $btnRefresh.Add_Click({
                        ...
                        $job = Start-Job -ScriptBlock {
                            ...
                            $resSoft = @(if ($rawSoft) { foreach ($r in $rawSoft) { [PSCustomObject]@{ Name = $r.Name; Version = $r.Version; Type = $r.Type; Identifier = $r.Identifier } } })
                            ...
                            if ($result -and $result.ExitCode -eq 0 -and $result.Lines -and $result.Lines.Count -gt 1) {
                                ...
                                for ($i = 1; $i -lt $result.Lines.Count; $i++) {
                                    ...
                                    try {
                                        ...
                                        if ($uN -and $sId) { [PSCustomObject]@{ Username=$uN; SessionId=$sId; State=$st } }
