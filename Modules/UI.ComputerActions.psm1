                        ...
                        if ($job.State -eq 'Completed') {
                            ...
                            if ($res -and $res.Success) { 
                                try {
                                    ...
                                    $gpoList = @(
                                        Parse-GPONodes -nodes ($xml.SelectNodes("//*[local-name()='ComputerResults']/*[local-name()='GPO']")) -scope "Computer" -xmlDoc $xml
                                        Parse-GPONodes -nodes ($xml.SelectNodes("//*[local-name()='UserResults']/*[local-name()='GPO']")) -scope "User" -xmlDoc $xml
                                    )
                                                ...
                                                if ($tvDetails) {
                                                    ...
                                                    function Build-Tree {
                                                        ...
                                                        if ($node.HasAttributes) {
                                                            $attrs = @(foreach ($a in $node.Attributes) { "$($a.Name)='$($a.Value)'" })
            ...
            $pollTick = {
                ...
                if ($job.State -ne 'Running' -or $elapsed -ge 12) {
                    ...
                    if ($job.State -eq 'Completed') {
                        ...
                        $sessions = @(if ($result -and $result.ExitCode -eq 0 -and $result.Lines -and $result.Lines.Count -gt 1) {
