    ...
    $scriptBlock = {
        ...
        try {
            if ($targetType -eq 'AppX') {
                # 🛡️ Sentinel: Prevent command injection by using Base64 for the payload and script
                $b64TargetId = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($targetId))
                $psCmd = "`$id=[System.Text.Encoding]::Unicode.GetString([Convert]::FromBase64String('$b64TargetId')); Get-AppxPackage -Name `"*`$id*`" -AllUsers | Foreach {Add-AppxPackage -DisableDevelopmentMode -Register `"`$(`$_.InstallLocation)\AppXManifest.xml`"}"
    ...
    $scriptBlock = {
        ...
        try {
            if ($targetType -eq 'AppX') {
                ...
                # 🛡️ Sentinel: Prevent command injection by using Base64 for the payload and script
                $b64TargetId = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($targetId))
                $psCmd = "`$id=[System.Text.Encoding]::Unicode.GetString([Convert]::FromBase64String('$b64TargetId')); Remove-AppxPackage -Package `$id -AllUsers"
                $encodedCmd = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($psCmd))
                Start-Process powershell.exe -ArgumentList @("-NonInteractive", "-WindowStyle", "Hidden", "-EncodedCommand", $encodedCmd) -Wait -WindowStyle Hidden
            } else {
                $cmd = $targetId
