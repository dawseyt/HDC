    ...
    <script>
        ...
        function renderLogTable() {
            ...
            const htmlParts = displayData.slice(0, limit).map(log => {
                ...
                const safeEvent = escapeHtml(log.event);
                const safeUsername = escapeHtml(log.username);
                const safeOperator = escapeHtml(log.operator);

                return `<tr><td class="px-6 py-2 whitespace-nowrap text-slate-500">${log.dateObj.toLocaleString()}</td><td class="px-6 py-2"><span class="${color} px-2 py-0.5 rounded text-[10px] uppercase border border-opacity-20 border-current">${safeEvent}</span></td><td class="px-6 py-2 font-bold">${safeUsername}</td><td class="px-6 py-2 text-slate-500">${safeOperator}</td></tr>`;
            });
        ...
        function renderDashboard(isInit = true) {
            ...
            if (activeCount === 0) {
                ...
                const rowsHtml = adLockouts.map(item => {
                    ...
                    const safeSamAccountName = escapeHtml(item.SamAccountName);
                    const safeLockoutTime = escapeHtml(lockoutTime);
                    const safeLockoutSource = escapeHtml(lockoutSource);

                    return `<tr class="hover:bg-slate-50">
                        <td class="px-6 py-4 font-bold text-slate-800">${safeSamAccountName}</td>
                        <td class="px-6 py-4 text-slate-600">${safeLockoutTime}</td>
                        <td class="px-6 py-4">
